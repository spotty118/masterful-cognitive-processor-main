...
export class ThinkingModelStrategyFactory {
     ...
     */
    static getStrategy(modelName, compositeConfig) {
    ...
    static createStrategy(modelName, compositeConfig) {
     ...
     */
    static createCompositeStrategy(model, config) {
        ...
        if (mode === 'weighted' && weights.length === strategies.length) {
            for (let i = 0; i < strategies.length; i++) {
                ...
                composite.strategies[i].weight = weights[i];
            }
        }
        return composite;
    }
    // Store the strategy selector instance
    static strategySelector = null;
    /**
     ...
     */
    static getOptimalStrategy(problem) {
     ...
     * @private
     */
    static selectByKeywords(problem) {
     ...
     * @private
     */
    static async improveStrategySelectionAsync(problem, selectedStrategy) {
        try {
     ...
     */
    static async getOptimalStrategyAsync(problem) {
        // Initialize strategy selector if needed
        if (!this.strategySelector) {
            await this.initializeStrategySelector();
        }
     ...
     * @private
     */
    static async initializeStrategySelector() {
        try {
...
//# sourceMappingURL=ThinkingModelStrategyFactory.js.map