...
const geminiService = new GeminiService(openRouterApiKey, 'pro', // Use Pro model by default 
...
thinkingService = new ThinkingServiceImpl(ServiceFactory, tokenOptimizer, mcpConfig, geminiService);
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                case 'thinking_process': {
                    try {
                        ...
                        if (!typedArgs.problem) {
                            throw new McpError(ErrorCode.InvalidParams, "The 'problem' field is required and must be a non-empty string");
                        }
                        if (typeof typedArgs.problem !== 'string' || typedArgs.problem.trim() === '') {
                            throw new McpError(ErrorCode.InvalidParams, "The 'problem' field must be a non-empty string");
                        }
                ...
                case 'update_token_metrics': {
                    ...
                    // Type assertion for args
                    const typedArgs = args;
                    tokenOptimizer.updateTokenMetrics(typedArgs.problem_id, typedArgs.estimated_tokens, typedArgs.actual_tokens, typedArgs.model);
                    return {
                        content: [
                            {
                                type: 'text',
