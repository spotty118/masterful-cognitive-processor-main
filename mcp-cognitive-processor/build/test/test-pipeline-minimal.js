...
async function testPipelineWithMetrics() {
    ...
    try {
      ...
      Analyze its time and space complexity, and suggest optimizations.`;
        ...
        const avgCoherence = steps.reduce((sum, step) => sum + (step.metrics?.coherence || 0), 0) / steps.length;
        const avgComplexity = steps.reduce((sum, step) => sum + (step.metrics?.complexity || 0), 0) / steps.length;
        const avgSignificance = steps.reduce((sum, step) => sum + (step.metrics?.significanceScore || 0), 0) / steps.length;
...
//# sourceMappingURL=test-pipeline-minimal.js.map