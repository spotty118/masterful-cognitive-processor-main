...
function testThinkingEngineState() {
    ...
    console.log('Previous reasoning contains expected content:', previousReasoning.includes(step1.reasoning) &&
        previousReasoning.includes(step2.reasoning));
    // Test getProblem method
    const problem = state.getProblem();
    console.log('Problem retrieved correctly:', problem === 'How can we optimize performance in a large React application?');
    return state;
}
/**
...
function testScoreCalculations() {
    ...
    function calculateTermOverlap(setA, setB) {
...
runTests();
//# sourceMappingURL=test-thinking-engine.js.map