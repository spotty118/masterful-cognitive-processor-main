...
try {
    const geminiService = DIServiceFactory.getService('geminiService');
...
const thinkingService = DIServiceFactory.getService('thinkingService');
if (!thinkingService) {
...
console.log('- Thinking Models:', mcpConfig.core.thinkingModels.map((m) => m.name).join(', '));
console.log('- Reasoning Systems:', mcpConfig.core.intelligence.reasoningSystems.map((r) => r.name).join(', '));
// Export the configured services
export { thinkingService, 
// Don't re-export tokenOptimizer to avoid circular dependencies
tokenHistoryService, mcpConfig };
...
class MCPCognitiveProcessor {
    ...
    constructor() {
        ...
        this.thinkingService = DIServiceFactory.getService('thinkingService');
        this.memoryService = DIServiceFactory.getService('memoryService');
        this.cacheService = DIServiceFactory.getService('cacheService');
        this.tokenOptimizer = DIServiceFactory.getService('tokenOptimizer');
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (uri === 'mcp://memory/stats') {
                ...
                const stats = {
                    ...
                    byType: {
                        working: items.filter((item) => item.id.includes('working')).length,
                        episodic: items.filter((item) => item.id.includes('episodic')).length,
                        semantic: items.filter((item) => item.id.includes('semantic')).length,
                        procedural: items.filter((item) => item.id.includes('procedural')).length,
                    },
                };
                return {
                    contents: [
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                ...
                case 'update_token_metrics': {
                    ...
                    // Type assertion for args
                    const typedArgs = args;
                    this.tokenOptimizer.updateTokenMetrics(typedArgs.problem_id, typedArgs.estimated_tokens, typedArgs.actual_tokens, typedArgs.model);
                    return {
                        content: [
                            {
                                type: 'text',
