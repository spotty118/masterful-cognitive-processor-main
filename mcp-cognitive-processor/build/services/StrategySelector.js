...
export class StrategySelector {
     ...
     * @private
     */
    simpleKeywordSelection(problem) {
        const problemLower = problem.toLowerCase();
     ...
     */
    updateStrategyEffectiveness(strategyName, effectivenessScore) {
    ...
    calculateUncertainty(text) {
        ...
        const uncertaintyScore = Math.min(uncertaintyMarkers.filter(marker => text.includes(marker)).length / 3, 1);
        const confidenceScore = Math.min(confidenceMarkers.filter(marker => text.includes(marker)).length / 3, 1);
    ...
    calculateSequentiality(text) {
        ...
        return Math.min((markerCount / 5) + explicitStepByStep + hasNumberedList, 1);
    }
    /**
    ...
    calculateAbstractionLevel(text) {
        ...
        const concreteScore = Math.min(concreteTerms.filter(term => text.includes(term)).length / 3, 1);
        const abstractScore = Math.min(abstractTerms.filter(term => text.includes(term)).length / 3, 1);
     ...
     * @private
     */
    calculateDomainSpecificity(text) {
        const domains = [
        ...
        const maxHitsDomain = domainHits.reduce((max, domain) => domain.hits > max.hits ? domain : max, { name: '', hits: 0 });
