...
export class ThinkingServiceImpl {
    serviceFactory;
    tokenOptimizer;
    config;
    processConfig;
    activeProcesses;
    aiService;
    constructor(serviceFactory, tokenOptimizer, config, aiService) {
        if (!aiService) {
    ...
    async initiateThinkingProcess(config) {
        ...
        try {
            ...
            const hybridService = {
                ...
                selectReasoningSystem: async (problem) => ({
                    name: 'default',
            ...
            try {
                ...
                openRouterService = this.serviceFactory.getService('openRouterService');
            ...
            const thinkingEngine = new ThinkingEngine(this.config, this.tokenOptimizer, hybridService, // Cast to any to bypass the type checker's confusion
            ...
            if (config.include_visualization) {
                visualization = strategy.generateVisualization(process.steps);
            }
    ...
    async getThinkingHistory(limit = 100) {
        try {
    ...
    selectThinkingModel(requestedModel, problem) {
    ...
    assessProblemComplexity(problem) {
        ...
        const keywordCount = complexityKeywords.reduce((count, keyword) => count + (problem.toLowerCase().includes(keyword) ? 1 : 0), 0);
        complexity += keywordCount * 0.1;
        // Structure-based complexity
        if (problem.includes('?'))
            complexity += 0.1;
        if (problem.includes('if'))
            complexity += 0.1;
        if (problem.includes('when'))
            complexity += 0.1;
        return Math.min(1, complexity);
    }
    async storeProcessHistory(process) {
        try {
     ...
     */
    async integrateCodeAnalysis(context) {
        try {
    ...
    extractCodeSnippets(context) {
        ...
        if (context.codeBlocks && Array.isArray(context.codeBlocks)) {
            snippets.push(...context.codeBlocks.map((block) => ({
     ...
     */
    transformCodeSuggestions(analysisResults) {
        return analysisResults.flatMap(result => (result.recommendations || []).flatMap((rec) => 
    ...
    shouldAnalyzeCode(request) {
        ...
        const checkText = (text) => Boolean(text && codeIndicators.some(indicator => text.toLowerCase().includes(indicator)));
        return (checkText(request.problem) ||
...
//# sourceMappingURL=ThinkingServiceImpl.js.map