 ...
 */
const ensureVectorEmbeddings = async () => {
    ...
    if (itemsWithoutVectors.length === 0)
        return;
...
const saveVectorsToDisk = async () => {
    try {
        ...
        await fs.promises.writeFile(path.join(VECTOR_DIR, 'vectors.json'), JSON.stringify(vectors, null, 2));
    }
    catch (error) {
 ...
 */
export const storeMemory = async (item) => {
 ...
 */
const saveMemoryVector = async (itemId, vector) => {
    const memoryVector = {
        itemId,
        vector,
        created: new Date().toISOString()
 ...
 */
export const retrieveMemory = async (query, limit = 10) => {
    await initializeMemory();
    if (!query) {
 ...
 */
export const getMemoryItemsByType = async (type) => {
    await initializeMemory();
 ...
 */
export const updateMemoryConnections = async (itemId, connections) => {
    await initializeMemory();
 ...
 */
export const getConnectedMemories = async (itemId) => {
    await initializeMemory();
    const item = memoryItems.get(itemId);
    if (!item) {
        return [];
 ...
 */
export const performMemoryMaintenance = async () => {
    await initializeMemory();
    let cleanedItems = 0;
    try {
        ...
        for (const file of vectorFiles) {
            if (!file.endsWith('.json') || file === 'vectors.json')
                continue;
            const id = file.replace('.json', '');
            if (!memoryItems.has(id)) {
 ...
 */
const calculateTextRelevance = (query, item) => {
