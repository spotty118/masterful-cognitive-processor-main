  ...
  try {
    ...
    for (const step of steps) {
      ...
      if (step.metrics.coherence === undefined) {
        ...
        try {
          step.metrics.coherence = await (engine as any).calculateCoherenceScore(
            steps,
            step,
            'How to optimize a recursive algorithm for calculating Fibonacci numbers?'
          );
      ...
      if (step.metrics.complexity === undefined) {
        ...
        try {
          step.metrics.complexity = await (engine as any).calculateComplexityScore(step.reasoning);
      ...
      if (step.metrics.significanceScore === undefined) {
        ...
        try {
          step.metrics.significanceScore = await (engine as any).calculateSignificanceScore(
            step.reasoning,
            'How to optimize a recursive algorithm for calculating Fibonacci numbers?'
          );
