...
// Define a scoring interface for strategy selection
...
export class AdvancedStrategySelector implements IReasoningStrategy {
  ...
  constructor(config: MCPConfig) {
    ...
    try {
      ...
      if (defaultModel?.maxTokens !== undefined) {
        this.resourceConstraints.maxTokens = Math.min(
          this.resourceConstraints.maxTokens,
          Number(defaultModel.maxTokens)
        );
      }
    } catch (error) {
   ...
   */
  async selectReasoningSystem(problem: string): Promise<{
    name: string;
    description: string;
    implementation: string;
  }> {
    ...
    let selectedStrategy: StrategyScore;
   ...
   */
  private async analyzeProblemComplexity(problem: string): Promise<ProblemComplexity> {
   ...
   */
  private async identifyProblemDomain(problem: string): Promise<ProblemDomain> {
    // Lowercase for consistent matching
    const text = problem.toLowerCase();
    
    // Define domain-specific keywords
    const domainKeywords: Record<ProblemDomain, RegExp> = {
    ...
    for (const [domain, regex] of Object.entries(domainKeywords)) {
      ...
      if (matches > maxMatches) {
        maxMatches = matches;
        bestMatch = domain as ProblemDomain;
      }
    }
   ...
   */
  private async extractProblemFeatures(problem: string): Promise<string[]> {
  ...
  private async scoreStrategies(
    strategies: Array<{ name: string; description: string; implementation: string }>,
    criteria: {
      complexity: ProblemComplexity;
      domain: ProblemDomain;
      features: string[];
      resourceConstraints: { maxTokens: number; maxTime: number; parallelCapacity: number };
    }
