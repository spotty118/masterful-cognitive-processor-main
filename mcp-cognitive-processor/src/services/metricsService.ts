...
export const recordMetric = (
  ...
  let series = (metrics[category] as MetricSeries[]).find(s => s.name === name);
  ...
  if (!series) {
    ...
    (metrics[category] as MetricSeries[]).push(series);
  } else if (metadata) {
 ...
 */
export const getMetrics = (category: keyof PerformanceMetrics): MetricSeries[] => {
  return (metrics[category] as MetricSeries[]) || [];
};
...
export const getMetricStats = (
  category: keyof PerformanceMetrics, 
  name: string
): { min: number; max: number; avg: number; median: number } | null => {
  const series = (metrics[category] as MetricSeries[])?.find(s => s.name === name);
 ...
 */
export const performMetricsMaintenance = (): number => {
  try {
    let prunedCount = 0;
    
    // Remove old points from all series
    for (const category of Object.keys(metrics) as Array<keyof PerformanceMetrics>) {
      ...
      for (const series of metrics[category]) {
        ...
        const recentPoints = series.points.filter(point => 
          new Date(point.timestamp) >= thirtyDaysAgo
        );
        
        // For older points, keep one per month
        const olderPoints = series.points.filter(point => 
          new Date(point.timestamp) < thirtyDaysAgo
        );
        
        // Group older points by month
        const pointsByMonth: Record<string, MetricPoint[]> = {};
        ...
        const sampledOlderPoints: MetricPoint[] = [];
