...
import { TokenHistory, TokenPredictionResult } from '../models/types.js';
...
let tokenHistory: Map<string, TokenHistory> = new Map();
let isInitialized = false;

/**
 * Initializes the token history system
 */
const initializeTokenHistory = async (): Promise<void> => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const historyItem = JSON.parse(data) as TokenHistory;
 ...
 */
export const predictTokenUsage = async (
  prompt: string,
  model?: string
): Promise<TokenPredictionResult> => {
  await initializeTokenHistory();
  
  const queryHash = createHash('md5').update(prompt).digest('hex');
  const similarQueries: Array<{ queryHash: string; similarity: number }> = [];
  
  // Find similar queries in history
  for (const [id, history] of tokenHistory.entries()) {
    if (model && !id.startsWith(model)) continue;
...
export const getTokenStats = async (): Promise<{
  ...
  let totalTokens = 0;
  const patterns = new Map<string, { occurrences: number; totalTokens: number }>();
 ...
 */
export const performTokenHistoryMaintenance = async (): Promise<number> => {
  await initializeTokenHistory();
  let cleanedItems = 0;
