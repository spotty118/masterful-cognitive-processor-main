...
interface StrategyWeight {
  strategy: IThinkingModelStrategy;
  weight: number;
  name: string;
}
...
interface ThinkingStepWithMetadata extends ThinkingStep {
...
export class CompositeStrategy extends BaseThinkingStrategy {
  ...
  private problemContext: string = '';
  private feedbackEnabled: boolean = true;
  private strategyPerformance: Map<string, StrategyPerformance> = new Map();
  private currentConfidence: number = 0.5;
  private confidenceReasoning: string = '';
  private alternativePaths: AlternativePath[] = [];

  constructor(
    model: ThinkingModel, 
    tokenOptimizer: ITokenOptimizer,
    strategies: IThinkingModelStrategy[] = [],
    mode: 'sequential' | 'parallel' | 'weighted' = 'sequential'
  ) {
   ...
   */
  addStrategy(strategy: IThinkingModelStrategy, weight: number = 1.0): void {
  ...
  async initialize(problem: string): Promise<void> {
    ...
    const initPromises = this.strategies.map(({ strategy }) => 
   ...
   */
  private async nextStep(): Promise<ThinkingStepWithMetadata | null> {
   ...
   */
  private async sequentialNextStep(): Promise<ThinkingStepWithMetadata | null> {
    ...
    for (const { strategy, name } of this.strategies) {
      try {
        ...
        if (step) {
          (step as ThinkingStepWithMetadata).metadata = { ...((step as ThinkingStepWithMetadata).metadata || {}), sourceStrategy: name };
          this.steps.push(step);
          
          // Update strategy performance if feedback is enabled
          if (this.feedbackEnabled) {
            this.updateStrategyPerformance(name, step as ThinkingStepWithMetadata);
          }
          
          return step as ThinkingStepWithMetadata;
        }
      } catch (error) {
   ...
   */
  private async parallelNextStep(): Promise<ThinkingStepWithMetadata | null> {
    ...
    const validResults = results.filter(result => result !== null) as Array<{ step: ThinkingStepWithMetadata, name: string }>;
    ...
    if (selected) {
      (selected.step as ThinkingStepWithMetadata).metadata = { ...((selected.step as ThinkingStepWithMetadata).metadata || {}), sourceStrategy: selected.name };
   ...
   */
  private async weightedNextStep(): Promise<ThinkingStepWithMetadata | null> {
    ...
    let selectedStrategy: StrategyWeight | null = null;
    ...
    if (selectedStrategy) {
      ...
      if (step) {
        (step as ThinkingStepWithMetadata).metadata = { ...((step as ThinkingStepWithMetadata).metadata || {}), sourceStrategy: selectedStrategy.name };
        this.steps.push(step);
   ...
   */
  private selectBestStep(candidates: Array<{ step: ThinkingStepWithMetadata, name: string }>): { step: ThinkingStepWithMetadata, name: string } | null {
    if (candidates.length === 0) return null;
    if (candidates.length === 1) return candidates[0];
    
    // If step has confidence score, use that
    const withConfidence = candidates.filter(c => 
      c.step.metadata && typeof c.step.metadata.confidence === 'number'
    );
    
    if (withConfidence.length > 0) {
      return withConfidence.reduce((best, current) => {
        const currentConfidence = current.step.metadata?.confidence as number;
        const bestConfidence = best.step.metadata?.confidence as number;
   ...
   */
  private updateStrategyPerformance(strategyName: string, step: ThinkingStepWithMetadata): void {
  ...
  getProgress(): number {
    ...
    const totalProgress = this.strategies.reduce(
      (sum, { strategy }) => sum + strategy.getProgress(),
      0
    );
  ...
  generateVisualization(steps?: ThinkingStep[]): ThinkingVisualization {
    ...
    const nodes = stepsToVisualize.map((step, index) => {
      ...
      const metadata = (step as ThinkingStepWithMetadata).metadata || {};
  ...
  async generateAlternativePaths(count: number): Promise<AlternativePath[]> {
    ...
    const allPaths: AlternativePath[] = [];
  ...
  async compareAlternativePaths(): Promise<{
    ...
    const allTradeoffs: { [key: string]: string } = {};
  ...
  protected async calculateTokenEfficiency(): Promise<number> {
    ...
    const averageReasoning = this.steps.reduce(
      (sum, step) => sum + (step.reasoning ? step.reasoning.length : 0), 0
    ) / this.steps.length;
   ...
   */
  private getStrategyMetricsInternal(): any {
    const metrics: any = {
   ...
   */
  private calculateAverageTokens(): number {
   ...
   */
  private async getAlternativePathsInternal(): Promise<AlternativePath[]> {
   ...
   */
  private async visualize(): Promise<ThinkingVisualization[]> {
    // Create visualizations for the composite strategy
    const allVisualizations: ThinkingVisualization[] = [];
    ...
    if (this.feedbackEnabled && this.steps.length > 0) {
      ...
      for (const step of this.steps) {
        const stepWithMeta = step as ThinkingStepWithMetadata;
        if (stepWithMeta.metadata && stepWithMeta.metadata.sourceStrategy) {
          const source = stepWithMeta.metadata.sourceStrategy as string;
