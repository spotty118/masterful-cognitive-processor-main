...
export class DIServiceFactory {
  ...
  public static async initialize(config?: Partial<MCPConfig>): Promise<void> {
   ...
   */
  private static initializeAIServices(): void {
   ...
   */
  private static async initializeOpenRouterServices(apiKey: string): Promise<void> {
   ...
   */
  private static initializeProcessingPipeline(): void {
    ...
    try {
      ...
      const googleFlashService = this.container.get<IAIService>('googleFlashService');
      const geminiProService = this.container.get<IAIService>('geminiProService');
      const deepSeekService = this.container.get<IAIService>('deepSeekService');
      
      // Try to get Claude service (from direct API or OpenRouter)
      let claudeService: IAIService;
      if (this.container.has('claudeService')) {
        claudeService = this.container.get<IAIService>('claudeService');
        console.log('Using direct Claude API for pipeline');
      } else if (this.container.has('claudeOpenRouterService')) {
        claudeService = this.container.get<IAIService>('claudeOpenRouterService');
  ...
  private static initializeModelFallback(): void {
    ...
    const registeredServices: {[key: string]: { priority: number, weight: number }} = {
    ...
    Object.entries(registeredServices).forEach(([name, config]) => {
      try {
        ...
        if (this.isAIService(service)) {
          fallbackService.registerProvider(
            name,
            service,
            config.priority,
            config.weight
  ...
  private static initializeThinkingService(): void {
    const config = this.container.get<ConfigurationService>('configService').getConfig();
    const tokenOptimizer = this.container.get<TokenOptimizerImpl>('tokenOptimizer');

    // Get the Gemini service
    let aiService: IAIService;
    try {
      aiService = this.container.get<IAIService>('geminiService');
    ...
    const thinkingService = new ThinkingServiceImpl(
      ServiceFactory, // Pass ServiceFactory class directly
      tokenOptimizer,
      config,
      aiService
    );
