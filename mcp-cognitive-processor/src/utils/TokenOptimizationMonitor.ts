...
export class TokenOptimizationMonitor {
    private static instance: TokenOptimizationMonitor;
    ...
    public static getInstance(): TokenOptimizationMonitor {
    ...
    private updateModelStats(metric: TokenMetric): void {
    ...
    public getModelStats(model: string): {
        totalSavings: number;
        totalOptimizations: number;
        averageSavingsPercent: number;
    } {
    ...
    public getAllModelStats(): Map<string, {
        totalSavings: number;
        totalOptimizations: number;
        averageSavingsPercent: number;
    }> {
    ...
    public getRecentMetrics(hours: number = 24): TokenMetric[] {
    ...
    public getOptimizationTrends(): {
        ...
        const now = Date.now();
        const hourly: OptimizationMetrics[] = [];
        const daily: OptimizationMetrics[] = [];
    ...
    public getContextualPerformance(): Map<string, {
        ...
        const contextStats = new Map<string, {
            optimizations: number;
            totalSavings: number;
            modelSavings: Map<string, number>;
        }>();

        // Calculate stats for each context
        for (const metric of this.metrics) {
            if (!metric.context) continue;

            const stats = contextStats.get(metric.context) || {
                optimizations: 0,
                totalSavings: 0,
                modelSavings: new Map<string, number>()
            };
